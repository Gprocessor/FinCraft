...
git checkout -- js/config.js 2>>"\$LOG" || true
git fetch origin "$CURRENT_BRANCH" --quiet >>"\$LOG" 2>&1
LOCAL=\$(git rev-parse HEAD)
REMOTE=\$(git rev-parse "origin/$CURRENT_BRANCH")
if [ "\$LOCAL" != "\$REMOTE" ]; then
  git pull --ff-only origin "$CURRENT_BRANCH" >>"\$LOG" 2>&1
  "\$DEPLOY_DIR/regen-frontend-config.sh" "\$REPO_ROOT" --reload >>"\$LOG" 2>&1
  (cd "\$DEPLOY_DIR" && docker compose up -d) >>"\$LOG" 2>&1
...
crontab -l 2>/dev/null | grep -v "$DEPLOY_DIR/backup.sh" \
  ...
  | grep -v "$DEPLOY_DIR/renew-cert.sh" > "$CRON_TMP" || true
